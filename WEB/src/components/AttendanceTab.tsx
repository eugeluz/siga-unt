...
import { collection, query, where, getDocs, doc, deleteDoc, writeBatch, addDoc, updateDoc, onSnapshot, setDoc } from 'firebase/firestore';
...
export const AttendanceTab: React.FC<AttendanceTabProps> = ({ cursos, fechas, alumnos = [] }) => {
  ...
  const searchAsistencia = async () => {
    ...
    try {
      ...
      const idCursoQ = courseObjForQuery?.idCurso;
      const fechaDocQ = asistenciaFechasFiltradas.find(f => f.inicio === asistenciaFecha) || currentFechaObj;
      const fechaIdQ = fechaDocQ?.id;
      console.log('Querying inscripciones for:', { idCurso: idCursoQ, fechaId: fechaIdQ });
      const alumnoMap = new Map((alumnos || []).map((a: any) => [String(a?.dni), a]));

      // Consultas de un solo campo (sin índice compuesto): normalizados por
      // idCurso (+ filtro fechaId en memoria) y legacy por nombre de curso
      // (+ filtro fechaInicio en memoria) para el período de transición.
      const seen = new Set<string>();
      const collected: any[] = [];
      const collect = (snap: any, accept: (r: any) => boolean) => {
        snap.docs.forEach((d: any) => {
          if (seen.has(d.id)) return;
          const r: any = d.data();
          if (!accept(r)) return;
          seen.add(d.id);
          collected.push({ snapId: d.id, data: r });
        });
      };
      if (idCursoQ !== undefined && idCursoQ !== null && String(idCursoQ).trim() !== '') {
        collect(
          await getDocs(query(collection(db, 'inscripciones'), where('idCurso', '==', (idCursoQ as any)))),
          (r: any) => !fechaIdQ || String(r.fechaId ?? '') === String(fechaIdQ)
        );
        if (String(idCursoQ).trim() !== '') {
          collect(
            await getDocs(query(collection(db, 'inscripciones'), where('idCurso', '==', String(idCursoQ)))),
            (r: any) => !fechaIdQ || String(r.fechaId ?? '') === String(fechaIdQ)
          );
        }
      }
      collect(
        await getDocs(query(collection(db, 'inscripciones'), where('curso', '==', asistenciaCurso))),
        (r: any) => String(r.fechaInicio || '') === String(asistenciaFecha)
      );
      if (courseObjForQuery && courseObjForQuery.curso && courseObjForQuery.curso !== asistenciaCurso) {
        collect(
          await getDocs(query(collection(db, 'inscripciones'), where('curso', '==', courseObjForQuery.curso))),
          (r: any) => String(r.fechaInicio || '') === String(asistenciaFecha)
        );
      }

      let list = collected.map(({ snapId, data }: any) => {
        // Nombres y contacto desde el padrón; snapshot de la inscripción como respaldo
        const al = alumnoMap.get(String(data.dni));
        const tel = al?.telPart || al?.telLab || data.telPart || data.telLab || '';
        return {
          id: snapId,
          ...data,
          apellido: al?.apellido || data.apellido || '',
          nombre: al?.nombre || data.nombre || '',
          email: al?.email || data.email || '',
          telefono: tel,
  ...
  const downloadPlanillaCertificados = async () => {
    ...
    const alumnoMapExp = new Map((alumnos || []).map((a: any) => [String(a?.dni), a]));
    const rowsWithEmail = aprobados.map((a) => {
      const sData = alumnoMapExp.get(String(a.dni));
      return {
        email: a.email || sData?.email || '',
        apellido: a.apellido || sData?.apellido || '',
        nombre: a.nombre || sData?.nombre || '',
        dni: a.dni || '',
