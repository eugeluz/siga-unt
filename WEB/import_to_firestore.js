...
function syncCursoResolucion(row, cursoNombre) {
    ...
    const resVal = rawRes ? String(rawRes).trim() : '';
    if (!cursoNombre) return { resolucion: resVal, idCurso: null };
    ...
    if (foundCurso) {
        if (resVal) foundCurso.resolucion = resVal;
        return { resolucion: resVal || foundCurso.resolucion || '', idCurso: foundCurso.idCurso };
    } else if (resVal || rowIdCurso) {
        ...
        return { resolucion: resVal, idCurso: newId };
    }
    return { resolucion: resVal, idCurso: null };
}

// Load AxC registrations (forma normalizada: la referencia a curso/fecha se
// resuelve en main(), luego de subir cursos y fechas)
rawAxC.forEach(row => {
    ...
    const { idCurso } = syncCursoResolucion(row, curso);
...
const inscripcionesRaw = Array.from(inscripcionesMap.values());
...
const backupData = {
    ...
    fechas,
    facultades,
    inscripciones: inscripcionesRaw
};
...
- Inscripciones: ${inscripcionesRaw.length}`);
...
if (fs.existsSync(serviceAccountPath)) {
    ...
    async function main() {
        try {
            ...
            // Fechas con auto-ID, capturando cada docId para referenciarlo desde inscripciones
            console.log(`Uploading ${fechas.length} items to collection "fechas"...`);
            const fechaIdByKey = new Map();
            for (const f of fechas) {
                const ref = await db.collection('fechas').add(f);
                fechaIdByKey.set(`${f.idCurso}||${f.inicio}`, ref.id);
            }
            console.log('Successfully uploaded collection "fechas"!');

            // Upload large collections
            await uploadCollection('alumnos', alumnos, 'dni');

            // Inscripciones normalizadas: { dni, idCurso, fechaId, resultado }
            // + snapshot apellido/nombre solo si el dni no está en el padrón
            const alumnoSet = new Set(alumnos.map(a => String(a.dni)));
            const cursoById = new Map(cursos.map(c => [Number(c.idCurso), c]));
            const cursoByName = new Map();
            cursos.forEach(c => {
                const n = String(c.curso || '').trim().toLowerCase();
                if (n && !cursoByName.has(n)) cursoByName.set(n, c);
            });
            const inscripciones = [];
            for (const r of inscripcionesRaw) {
                const cObj = (r.idCurso && cursoById.get(Number(r.idCurso))) ||
                    cursoByName.get(String(r.curso || '').trim().toLowerCase());
                if (!cObj) continue;
                const idCursoNum = Number(cObj.idCurso);
                const fechaId = fechaIdByKey.get(`${idCursoNum}||${r.fechaInicio}`);
                if (!fechaId) continue;
                const docData = {
                    dni: r.dni,
                    idCurso: idCursoNum,
                ...
                if (!alumnoSet.has(String(r.dni))) {
                    if (r.apellido) docData.apellido = r.apellido;
                    if (r.nombre) docData.nombre = r.nombre;
                }
                inscripciones.push(docData);
            }
