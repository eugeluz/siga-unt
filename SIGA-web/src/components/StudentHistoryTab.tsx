  ...
  const searchHistorial = async (dniToSearch?: string) => {
    ...
    try {
      ...
      const fullHistory = listInsc.map((insc: any) => {
        ...
        // Fecha: primero por docId (modelo normalizado), luego idCurso + inicio, luego nombre + inicio (legacy)
        const nombreCursoObj = cursoObj ? (cursoObj.nombreCompleto || cursoObj.curso) : undefined;
        const fechaObj = fechasList.find(f => (f as any).id && String((f as any).id) === String(insc.fechaId || ''))
          || fechasList.find(f => String(f.idCurso) === String(insc.idCurso ?? cursoObj?.idCurso) && f.inicio === insc.fechaInicio)
          || (insc.fechaInicio && nombreCursoObj
            ? fechasList.find(f => (f.curso || '') === nombreCursoObj && f.inicio === insc.fechaInicio)
            : undefined);
