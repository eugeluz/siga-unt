...
export const FacultiesTab: React.FC<FacultiesTabProps> = ({ facultades }) => {
  ...
  const searchFacultad = async () => {
    ...
    try {
      ...
      // Catálogos para resolver nombres de curso/fecha por IDs
      ...
      const fechasList = snapFechas.docs.map(d => ({ id: d.id, ...(d.data() as any) }));

      // 1) Alumnos del padrón cuya Unidad Académica coincide
      const alumnoSnaps = await Promise.all(
        uniqueNames.map(name =>
          getDocs(query(collection(db, 'alumnos'), where('unidadAcademica', '==', name)))
        )
      );
      const alumnoByDni = new Map<string, any>();
      alumnoSnaps.forEach(s => s.docs.forEach(d => {
        const a: any = d.data();
        if (a?.dni !== undefined && !alumnoByDni.has(String(a.dni))) alumnoByDni.set(String(a.dni), a);
      }));

      // 2) Inscripciones de esos DNIs (bloques de 10, un solo campo)
      const inscById = new Map<string, any>();
      const dnis = [...alumnoByDni.keys()].map(n => Number(n)).filter(n => n);
      for (let i = 0; i < dnis.length; i += 10) {
        const chunk = dnis.slice(i, i + 10);
        const s = await getDocs(query(collection(db, 'inscripciones'), where('dni', 'in', chunk)));
        s.docs.forEach(d => { if (!inscById.has(d.id)) inscById.set(d.id, d.data()); });
      }

      // 3) Respaldo transición: inscripciones legacy con unidadAcademica embebida
      // (cuyo alumno puede no estar en el padrón)
      for (const name of uniqueNames) {
        const s = await getDocs(query(collection(db, 'inscripciones'), where('unidadAcademica', '==', name)));
        s.docs.forEach(d => { if (!inscById.has(d.id)) inscById.set(d.id, d.data()); });
      }

      // 4) Filas con la misma forma de siempre (joins por IDs + snapshot)
      const results = [...inscById.values()].map((r: any) => {
        const v = displayInscripcion(r, { cursos: cursosList, fechas: fechasList, alumnos: alumnoByDni });
        return {
          dni: v.dni,
          apellido: v.apellido,
          nombre: v.nombre,
