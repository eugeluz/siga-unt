...
export const DashboardHome: React.FC<DashboardHomeProps> = ({
  ...
  inscripciones.forEach(ins => {
    const raw = String(ins.resultado || 'Cursando').trim().toLowerCase();
    let res = 'Cursando';
  ...
  // "Alumnos Aprobados": inscripciones en Aprobado, por curso
  // (una persona que aprobó 2 cursos cuenta 2 veces).
  const alumnosAprobados = inscripciones.filter(ins => {
    const r = (ins.resultado || '').trim().toLowerCase();
    return r === 'aprobado' || r === 'aprobada';
  }).length;
