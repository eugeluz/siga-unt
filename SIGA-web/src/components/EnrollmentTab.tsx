...
export const EnrollmentTab: React.FC<EnrollmentTabProps> = ({ cursos, fechas, facultades = [], alumnos = [] }) => {
  ...
  const executeLoteEnrollment = async () => {
    ...
    // Listas vivas: incluyen lo creado por este mismo lote para no duplicar.
    const cursosVivos: any[] = [...cursos];
    const fechasVivas: any[] = [...fechas];
    let nextCursoId = Math.max(0, ...cursos.map((c: any) => Number(c.idCurso) || 0)) + 1;
    ...
    try {
      ...
      for (const row of parsedLoteData) {
        ...
        // Filtro del Paso 1: si se eligió curso y/o fecha, solo se procesan
        // las filas que coincidan; las demás se omiten. Si el Paso 1 queda
        ...
        if (!cursoObjFila) {
          ...
          try {
            await setDoc(doc(db, 'cursos', String(nuevoId)), nuevoCurso);
            cursoObjFila = nuevoCurso;
            cursosVivos.push(nuevoCurso);
            stats.cursosCreados++;
          } catch (e) {
            console.error('Error creando curso:', e);
            skipped++;
            if (skippedExamples.length < 3) skippedExamples.push(`fila ${count}: DNI ${dniVal} no se pudo crear el curso (“${cursoEfectivo}”)`);
            continue;
          }
        }
        ...
        // La fecha se resuelve por ID Fecha del roundtrip o por inicio
        // normalizado (tolerando formatos); si no existe, se crea.
        ...
        if (idFechaCol) {
          const cand = fechasVivas.find((f: any) => String(f.id) === idFechaCol);
        ...
        if (!fechaObjFila) {
          ...
          fechaObjFila = fechasVivas.find((f: any) =>
        ...
        if (!fechaObjFila || !(fechaObjFila as any).id) {
          ...
          try {
            ...
            fechasVivas.push(fechaObjFila);
            stats.fechasCreadas++;
          } catch (e) {
            console.error('Error creando fecha:', e);
            skipped++;
            if (skippedExamples.length < 3) skippedExamples.push(`fila ${count}: DNI ${dniVal} no se pudo crear la fecha (“${cursoEfectivo}” ${fechaInicioFila})`);
            continue;
          }
        }
        ...
        // Clave normalizada: DNI + idCurso + fechaId. Se consulta por DNI
        // (sin índice compuesto) y se filtra en memoria; se conservan los
        // legacy (curso + fechaInicio) del período de transición.
        // Se sobrescribe (sin merge) para limpiar campos legacy; las
        // asistencias se unen y conservan.
        const seenIds = new Set<string>();
        const matchedDocs: any[] = [];
        const qDni = query(collection(db, 'inscripciones'), where('dni', '==', dniVal));
        const snapDni = await getDocs(qDni);
        snapDni.docs.forEach((d: any) => {
          const r: any = d.data();
          const sameNorm = String(r.idCurso ?? '') === String(idCursoValFila) && String(r.fechaId ?? '') === String(fechaIdFila);
          const sameLegacy = (r.curso || '') === cursoParaInscripcion && (r.fechaInicio || '') === fechaParaInscripcion;
          const sameLegacyAlt = !!(courseObjForQuery && courseObjForQuery.curso && courseObjForQuery.curso !== cursoParaInscripcion &&
            (r.curso || '') === courseObjForQuery.curso && (r.fechaInicio || '') === fechaParaInscripcion);
          if (!seenIds.has(d.id) && (sameNorm || sameLegacy || sameLegacyAlt)) {
            seenIds.add(d.id);
            matchedDocs.push(d);
          }
        });
        ...
        if (matchedDocs.length > 0) {
          // Unir asistencias de los duplicados (true gana) y conservarlas:
          // se sobrescribe el doc (sin merge) para limpiar campos legacy.
          const mergedAsist: Record<string, boolean> = {};
          matchedDocs.forEach((m: any) => {
            const a = (m.data() as any).asistencias || {};
            Object.keys(a).forEach(k => {
              if (a[k]) mergedAsist[k] = true;
              else if (!(k in mergedAsist)) mergedAsist[k] = false;
            });
          });
          ...
          await setDoc(doc(db, 'inscripciones', matchedDocs[0].id), enrollmentData);
          stats.updated++;
  ...
  // Exporta TODOS los inscriptos a Excel para revisión/limpieza, con las
  // mismas 9 columnas del lote histórico (resueltas por joins). El archivo
  // puede re-subirse por Lotes: las coincidencias DNI + Curso + Fecha se pisan
  // (se conservan las asistencias ya cargadas) y los duplicados se eliminan.
  const handleExportInscriptos = async () => {
    ...
    try {
      ...
      const alumnoMap = new Map((alumnos || []).map((a: any) => [String(a?.dni), a]));
      const rows = snap.docs.map(d => {
        const v = displayInscripcion(d.data(), { cursos, fechas, alumnos: alumnoMap });
        return {
          dni: v.dni ?? '',
          apellido: v.apellido,
          nombre: v.nombre,
        ...
        <div className="details-grid">
          ...
          <div className="details-box" style={{ height: 'fit-content' }}>
            <h3 style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              Paso 1: Destinatario de la Inscripción <span style={{ fontWeight: 400, fontSize: '0.8rem', color: 'var(--text-muted)' }}>(opcional)</span>
              <button type="button" onClick={() => alert({ title: 'Paso 1 — Destinatario (opcional)', message: 'Si el Excel trae columnas Programa / Curso / Fecha de inicio por fila, puede dejar este paso vacío y cada fila irá a su curso.\n\nSi elige curso y/o fecha aquí, SOLO se procesan las filas que coincidan (filtro); las demás se omiten.\n\nSi el Excel solo trae DNI | Apellido | Nombre | Condición, seleccione aquí el Programa → Curso → Fecha que se aplicará a todas las filas.', variant: 'info' })} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '2px', display: 'inline-flex', color: '#E8BC00' }} title="¿De qué se trata?"><HelpCircle size={16} /></button>
            </h3>
