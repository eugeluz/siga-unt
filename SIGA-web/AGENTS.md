...
- `inscripciones` NORMALIZADAS (auto-ID): `{dni, idCurso, fechaId, resultado}` + `asistencias` + snapshot `apellido`/`nombre` con los nombres del archivo. Al subir lotes, `alumnos` no se toca nunca (esa tabla solo se escribe desde la opción Alumnos); la vista muestra el nombre del padrón por DNI cuando existe y el snapshot del archivo en caso contrario; programa/carga/resolución pisan en `cursos` y cantidad pisa en `fechas` (solo si la fila trae valor distinto). Never write `curso`/`fechaInicio`/`email`/`unidadAcademica`/`cargoFuncion`/`area`/`ua`/`resolucion` there — those live in their own tables. Joins via `src/utils/inscripciones.ts` (`displayInscripcion`, `matchCurso`, `matchFecha`) with legacy fallback. Dedupe key = dni+idCurso+fechaId; query by `dni` only (no composite index) + in-memory filter. Overwrite (no merge) on re-upload to drop legacy fields, preserving merged `asistencias`. One-shot migration: `ConfigTab` → “Normalizar tabla de inscriptos”. Historic-lot Excel = 9 cols (DNI, Apellido, Nombre, Programa, Curso, Fecha de inicio, Carga horaria, Resolución, Condición) + opcionales ID Curso/ID Fecha (los escribe la exportación; al re-subir mandan sobre nombres/fechas, inmunes a que Excel reformatee fechas); fechas aceptan serial, serial-texto, ISO y latino vía `excelDateToJSDate` (`src/utils/date.ts`) — formato irreconocible = fila omitida, nunca fecha basura; export in `EnrollmentTab.handleExportInscriptos` + backup `02_inscriptos.xlsx` use the same cols for roundtrip. Botón “Reparar vínculos” (cabecera de Inscripción): cura links rotos (por ID, si no por nombre+fecha), fusiona duplicados misma persona+curso+fecha (conserva asistencias), borra fechas dup exactas sin uso; huérfanas sin curso se eliminan aparte con confirmación.
